...
if [[ "$TEST" == "FOO" ]]; then
  ...
  echo ERROR: Failed, wrong value forthe TEST variable. Please check the environment variable value inthe yml
  exit 1
fi;
...
if [[ "$secure" == "testingvariable" ]]; then
  ...
  echo ERROR: Failed, wrong value forthe secure variable. Please check the environment variable value inthe yml
  exit 1
fi;
